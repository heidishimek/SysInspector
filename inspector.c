 ...
 * @file
 *
 * System inspector: a Unix utility that inspects the system it runs on and
 * creates a live updating display using the 'proc' pseudo file system.
 *
...
int main(int argc, char *argv[])
{
    ...
    if (fd == -1) {
        fprintf(stderr, "Could not open proc directory: %s\n",
                options.procfs_loc);
        return 1;
    }
    close(fd);
